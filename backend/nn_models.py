...
class TransformerModel(nn.Module):
    def __init__(self, vocab, d_embed, nhead, d_hidden, d_out, num_layers, dropout=0.2):
        ...
        transformer_layers = nn.TransformerEncoderLayer(
            d_embed, nhead, d_hidden, dropout, batch_first=True
        )
...
class MultiModalThree(nn.Module):
    ...
    def forward(self, board, sequence, seq_lengths):
        ...
        seq_encoding = self.rnn(
            sequence, seq_lengths
        )  # [batch_size, seq_length, d_dimensions]
...
class MultiModalFive(nn.Module):
    def __init__(self, vocab, d_embed, d_hidden, d_out, dropout=0.5) -> None:
        ...
        self.cnn = SENet(128)
        self.fc = nn.Sequential(
            nn.Linear(64 + 128, 256), nn.ReLU(), nn.Linear(256, d_out)
        )
...
class MultiModalSix(nn.Module):
    def __init__(self, vocab, d_embed, d_hidden, d_out, dropout=0.5) -> None:
        super().__init__()
        self.rnn = RNNModelTwo(
            vocab, d_embed, d_hidden, 64, dropout=dropout, bidirectional=True
        )
        self.cnn = SENet(128)
        self.fc = nn.Sequential(
            nn.Linear(64 + 128, 256), nn.ReLU(), nn.Linear(256, d_out)
...
class MultiModalSeven(nn.Module):
    def __init__(self, vocab, d_embed, d_hidden, d_out, dropout=0.5) -> None:
        super().__init__()
        self.rnn = RNNModelTwo(
            vocab, d_embed, d_hidden, 128, dropout=dropout, bidirectional=True
        )
...
class MultiModalEight(nn.Module):
    def __init__(self, vocab, d_embed, d_hidden, d_out, dropout=0.5) -> None:
        super().__init__()
        nhead = 12
        self.transform = TransformerModel(
            vocab, d_embed, nhead, d_hidden, d_out, num_layers=3, dropout=0.1
        )
        self.cnn = SENetThree(256)
        self.fc = nn.Sequential(
            nn.Dropout(0.2),
            nn.Linear(256 + d_embed, 256),
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(256, d_out),
        )
...
#     d_embed = 64
#     model = MultiModalTwo()
#     model.load_state_dict(torch.load('multimodalmodel-best.pth'))
