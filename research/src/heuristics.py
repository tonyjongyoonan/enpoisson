...
def check_hanging(board, turn):
    ...
    for square in range(64):
        ...
        if piece is not None and piece.color != turn:
            if is_hanging(board, turn, square):
                print("SQUARE: ", square)
                # print(square)
...
def is_hanging(board, turn, square):
    ...
    piece = board.piece_at(square)
    if piece:
        attackers = board.attackers(turn, square)
        defenders = board.attackers(not turn, square)
        # print(attackers)
        # print(defenders)

        # if len(attackers) >= len(defenders) and board.is_attacked_by(board.turn, square):
        # print(len(attackers))
        # print(attackers)
        # print(len(defenders))
        # print(defenders)
        ...
        for attacker in attackers:
            attackers_sum += piece_to_point[board.piece_at(attacker).symbol()]
        ...
        if len(attackers) > 0 and len(attackers) > len(defenders):
            ...
            attacker_types = set()
            for attacker in attackers:
                attacker_types.add(board.piece_at(attacker).symbol())
            # find the piece types of the defenders
            defender_types = set()
            for defender in defenders:
...
def get_all_delta_for_move(board, move):
    ...
    number_hangers = check_hanging(board, board.turn, move)
    number_hangers_for_opp = check_hanging(board, not board.turn, move)
    ...
    print("Number of hanging pieces for opponent afer move ", move, ": ", number_hangers_for_opp)
...
def add_uci_to_board(board, uci):
